...
import { Insentif, PermitStatus, Users } from "@prisma/client";
...
export default function PagePermitInsentif() {
  ...
  const [insentifs, setInsentifs] = useState<Insentif[]>([]);
  ...
  const getData = async () => {
    ...
    await fetch(
      `/api/permit-insentif?${GenerateQueries(
        pageProps.page,
        pageProps.pageSize,
        pageProps.filters
  ...
  useEffect(() => {
    (async () => {
      await fetch("/api/insentif?page=1&pageSize=100")
        .then((res) => res.json())
        .then((res) => setInsentifs(res.data));
    })();
  }, []);
...
const DeletePermit = ({
  ...
  const handleOK = async () => {
    setLoading(true);
    await fetch("/api/permit-insentif?id=" + data.id, { method: "DELETE" })
      .then((res) => res.json())
      .then(async (res) => {
        if (res.status === 200) {
...
const UpsertPermit = ({
  ...
  const handleSubmit = async () => {
    setLoading(true);
    await fetch("/api/permit-insentif", {
  ...
  return (
    <div>
      <Modal
        ...
        onOk={() => handleSubmit()}
        okButtonProps={{
          disabled: !data.insentifId || !data.requesterId,
      ...
      >
        <div className="flex flex-col gap-2">
          <IFormInput
            label="Permohonan"
            classname="flex-1"
            value={data.insentifId}
            onChange={(e: any) => {
              const find = insentifs.filter((i) => i.id === e);
              setData({
                ...data,
                insentifId: e,
                Insentif: find[0],
                nominal:
                  find[0].insentifType === "NOMINAL"
                    ? find[0].insentif
                    : users.principalSalary * (find[0].insentif / 100),
              });
            }}
            type="options"
            option={insentifs.map((ins) => ({
              label: ins.name,
              value: ins.id,
            }))}
          />
...
const ProsesPermitAbsence = ({
  ...
  return (
    <div>
      <Modal
        open={open}
        title={"PROSES PERMOHONAN INSENTIF"}
        onCancel={() => setOpen(false)}
        onOk={() => handleSubmit()}
        loading={loading}
        style={{ top: 30 }}
